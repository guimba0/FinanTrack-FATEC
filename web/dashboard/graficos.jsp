...
<%
    ...
    // --- 2. DADOS LINHA (Fluxo Diário) ---
    double[] diasEntrada = new double[31]; double[] diasSaida = new double[31];
    for (Transacao t : transacoes) {
    ...
    StringBuilder lblDias = new StringBuilder("["); StringBuilder datEnt = new StringBuilder("["); StringBuilder datSai = new StringBuilder("[");
    int maxDias = LocalDate.of(ano, mes, 1).lengthOfMonth();
    for (int i = 0; i < maxDias; i++) {
        if (i > 0) { lblDias.append(","); datEnt.append(","); datSai.append(","); }
        lblDias.append("'").append(i+1).append("'");
        datEnt.append(diasEntrada[i]); datSai.append(diasSaida[i]);
    }
...
<% if (isMinimal) { %>
    ...
    <head>
        ...
        <style>
            /* Layout Fluido e sem margens extras */
...
<% } %>
    ...
    <div class="row g-3">
        <div class="col-lg-5 col-md-12">
            <div class="card card-dark h-100">
                ...
                <div class="card-body p-2 d-flex align-items-center justify-content-center">
                    <% if (mapCategorias.isEmpty()) { %><div class="text-muted small py-5">Sem dados.</div><% } else { %>
        ...
        <div class="col-lg-7 col-md-12">
            <div class="card card-dark h-100">
                <div class="card-header fw-bold text-center text-info py-2">Fluxo Diário</div>
    ...
    <script>
        Chart.defaults.color = '#e0e0e0'; Chart.defaults.borderColor = '#495057';
        ...
        if (ctxLine) {
            new Chart(ctxLine, {
                ...
                data: {
                    ...
                    datasets: [
                        { label: 'Entradas', data: <%= datEnt.toString() %>, borderColor: '#198754', backgroundColor: 'rgba(25, 135, 84, 0.1)', fill: true, yAxisID: 'y', tension: 0.4, pointRadius: 0, pointHoverRadius: 4 },
                        { label: 'Saídas', data: <%= datSai.toString() %>, borderColor: '#dc3545', backgroundColor: 'rgba(220, 53, 69, 0.1)', fill: true, yAxisID: 'y1', tension: 0.4, pointRadius: 0, pointHoverRadius: 4 }
                    ]
                },
                options: {
                    responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false },
                    scales: { x: { grid: { display: false }, ticks: { maxTicksLimit: 10 } }, y: { type: 'linear', display: true, position: 'left', grid: { color: '#343a40' } }, y1: { type: 'linear', display: true, position: 'right', grid: { drawOnChartArea: false } } },

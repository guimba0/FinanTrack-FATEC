                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <button class="btn btn-sm btn-outline-primary edit-btn"
                                        ...
                                        data-bs-target="#editContaModal"
                                        data-id="<%= conta.getId() %>"
                                        data-descricao="<%= conta.getDescricao() %>"
                                        data-valor="<%= conta.getValor() %>"
                                        data-categoria="<%= conta.getCategoria() %>"
                                        data-tipo-pagamento="<%= conta.getTipoPagamento() %>"
                                        data-dia-vencimento="<%= (conta.getDiaVencimento() != null ? conta.getDiaVencimento() : "") %>"
                                        data-data-inicio="<%= conta.getDataInicio().toString() %>"
                                        data-data-fim="<%= (conta.getDataFim() != null ? conta.getDataFim().toString() : "") %>">
...
<div class="modal fade" id="editContaModal" tabindex="-1" aria-labelledby="editContaModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-dark text-light">
            <div class="modal-header">
                <h5 class="modal-title" id="editContaModalLabel">Editar Conta Recorrente</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="${pageContext.request.contextPath}/conta-recorrente-editar" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="edit-tipoPagamento" class="form-label">Tipo de Pagamento</label>
                            <select class="form-select" name="tipoPagamento" id="edit-tipoPagamento" required>
                                <option value="Boleto">Boleto</option>
                                <option value="Debito Automatico">Débito Automático</option>
                            </select>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="edit-diaVencimento" class="form-label">Dia do Vencimento</label>
                            <input type="number" class="form-control" name="diaVencimento" id="edit-diaVencimento" min="1" max="31" placeholder="Para boletos">
                        </div>
                    </div>

                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="edit-dataInicio" class="form-label">Data de Início</label>
                            <input type="date" class="form-control" name="dataInicio" id="edit-dataInicio" required>
                        </div>
                        <div class="col-md-6 mb-3">
                             <label for="edit-dataFim" class="form-label">Data de Fim (Opcional)</label>
                            <input type="date" class="form-control" name="dataFim" id="edit-dataFim">
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const editFormTipoPagamento = document.getElementById('edit-tipoPagamento');
    const editFormDiaVencimento = document.getElementById('edit-diaVencimento');
    const editFormDataInicio = document.getElementById('edit-dataInicio');
    const editFormDataFim = document.getElementById('edit-dataFim');

    editButtons.forEach(button => {
        button.addEventListener('click', function () {
            // Pega os dados do botão (data-*)
            const id = this.dataset.id;
            const descricao = this.dataset.descricao;
            const valor = this.dataset.valor;
            const categoria = this.dataset.categoria;
            const tipoPagamento = this.dataset.tipoPagamento;
            const diaVencimento = this.dataset.diaVencimento;
            const dataInicio = this.dataset.dataInicio;
            const dataFim = this.dataset.dataFim;

            // Preenche o formulário do modal com os dados
            editFormId.value = id;
            editFormDescricao.value = descricao;
            editFormValor.value = valor;
            editFormCategoria.value = categoria;
            editFormTipoPagamento.value = tipoPagamento;
            editFormDiaVencimento.value = diaVencimento;
            editFormDataInicio.value = dataInicio;
            editFormDataFim.value = dataFim;
        });
    });
});
</script>

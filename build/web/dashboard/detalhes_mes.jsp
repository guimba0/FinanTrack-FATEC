...
<%
    ...
    double ent = 0, sai = 0;
    for (Transacao t : lista) { if ("entrada".equals(t.getTipo())) ent += t.getValor(); else sai += t.getValor(); }
...
<head>
    ...
    <style>
        ...
        .btn-left { left: 0; background: #198754; } /* Verde para Relatórios */
...
<body>
    ...
    <script>
        ...
        // --- FUNÇÃO EXPORTAR EXCEL (COM RESUMO) ---
        function exportarTabelaExcel() {
            ...
            for (var i = 0; i < rows.length; i++) {
                rows[i].deleteCell(-1); // Remove coluna ações
                if(i > 0) { 
                    var st = rows[i].cells[3]; st.innerHTML = st.getAttribute('data-status'); // Ajusta status
                }
            }

            // Cria Tabela de Cabeçalho com Resumo
            ...
            html = html.replace(/ /g, '%20'); // Encode spaces

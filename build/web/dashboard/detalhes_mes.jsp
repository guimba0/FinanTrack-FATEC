<%-- 
    Página: Detalhes do Mês (Extrato)
    Objetivo: Listar transações, saldo, permitir CRUD e visualizar gráficos.
--%>
...
<%
    ...
    if (pParam != null && !pParam.isEmpty()) {
        try { String[] parts = pParam.split("-"); ano = Integer.parseInt(parts[0]); mes = Integer.parseInt(parts[1]); } catch(Exception e){}
    } else {
    ...
    for (Transacao t : lista) { 
        if ("entrada".equals(t.getTipo())) ent += t.getValor();
        else sai += t.getValor(); 
    }
...
<head>
    ...
    <style>
        ...
        /* Sidebar de Gráficos (Off-canvas manual) */
...
<body>
    ...
    <%-- Botões Flutuantes --%>
    <button id="btnRelatorios" class="btn-float btn-left" data-bs-toggle="offcanvas" data-bs-target="#offRelatorios" title="Relatórios / Excel"><i class="bi bi-file-earmark-spreadsheet fs-4"></i></button>
    ...
    <div id="wrapper">
        <div id="main">
            <div class="container mt-4 mb-5">
                ...
                <%-- Barra de Navegação e Botões de Ação --%>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td data-status="<%= t.isPago() ? "Pago" : "Pendente" %>">
                                        <%-- ATUALIZADO: Link para TransacaoController?acao=status --%>
                                        <i class="<%= t.isPago() ? "bi bi-check-circle-fill text-success" : "bi bi-circle text-warning" %> fs-5 status-btn" 
                                           onclick="confirmarAcao('Alterar Status', 'Mudar status para <%= t.isPago() ? "Pendente" : "Pago" %>?', '${pageContext.request.contextPath}/TransacaoController?acao=status&id=<%= t.getId() %>&pago=<%= !t.isPago() %>&mes=<%= mes %>&ano=<%= ano %>')"></i>
                                    </td>
                                    ...
                                    <td class="text-center no-export">
                                        ...
                                        <%-- ATUALIZADO: Link para TransacaoController?acao=excluir --%>
                                        <button class="btn btn-link text-danger btn-sm p-0" onclick="confirmarAcao('Excluir', 'Apagar lançamento?', '${pageContext.request.contextPath}/TransacaoController?acao=excluir&id=<%= t.getId() %>&mes=<%= mes %>&ano=<%= ano %>')"><i class="bi bi-trash"></i></button>
                                    </td>
                                </tr>
                                <% } %>
                            </tbody>
    ...
    <div class="modal fade" id="modalLanc" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <%-- ATUALIZADO: Action aponta para TransacaoController --%>
                <form id="formL" action="${pageContext.request.contextPath}/TransacaoController" method="POST">
                    
                    <%-- Campos Ocultos de Controle --%>
                    <input type="hidden" name="acao" id="lacao" value="salvar"> <%-- Define se é salvar ou editar --%>
                    <input type="hidden" name="mes" value="<%= mes %>">
                    <input type="hidden" name="ano" value="<%= ano %>">
                    <input type="hidden" name="id" id="lid">
                    <input type="hidden" name="split" id="lsplit" value="false">

                    <div class="modal-header bg-dark text-white"><h5 class="modal-title" id="ltit">Novo</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
                    <div class="modal-body">
                        <div id="alertRec" class="alert alert-warning small mb-2" style="display:none"><i class="bi bi-info-circle"></i> Editando deste mês em diante.</div>
                        <div class="row g-2 mb-2"><div class="col-8"><input type="text" name="descricao" id="ldesc" class="form-control" placeholder="Descrição" required></div><div class="col-4"><input type="number" name="valor" id="lval" step="0.01" class="form-control" placeholder="0.00" required></div></div>
                        <div class="row g-2 mb-2"><div class="col-6"><input type="date" name="data" id="ldata" class="form-control" required></div><div class="col-6"><select name="tipo" id="ltipo" class="form-select"><option value="saida">Saída</option><option value="entrada">Entrada</option></select></div></div>
                        <div class="mb-2"><select name="categoria" id="lcat" class="form-select" onchange="verifOutro()"><option>Alimentação</option><option>Transporte</option><option>Moradia</option><option>Lazer</option><option>Saúde</option><option>Salário</option><option value="Outro">Outro...</option></select><input type="text" name="categoriaOutro" id="lout" class="form-control mt-1" style="display:none" placeholder="Qual?"></div>
                            <div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="lrec" name="recorrente" onchange="toggleRec()"><label class="form-check-label fw-bold" for="lrec">Conta Recorrente</label></div>
                            <div id="divFim" style="display:none" class="mt-2 small text-muted"><label>Data Fim (Opcional):</label><input type="date" name="dataFim" class="form-control form-control-sm"></div>
                            <div class="form-check mt-2" id="divPago"><input class="form-check-input" type="checkbox" name="pago" id="lpago" checked><label class="form-check-label" for="lpago">Já pago?</label></div>
                    </div>
                    <div class="modal-footer p-1"><button type="submit" class="btn btn-dark w-100">Salvar</button></div>
                </form>
            </div>
        </div>
    </div>

    <%-- Modal de Lista de Assinaturas --%>
    <div class="modal fade" id="modalRecorrentes" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header bg-primary text-white py-2"><h6 class="mb-0">Minhas Assinaturas</h6><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body p-0"><% if(recorrentes != null && !recorrentes.isEmpty()) { %><ul class="list-group list-group-flush"><% for(ContaRecorrente r : recorrentes) { %><li class="list-group-item d-flex justify-content-between align-items-center"><div><strong><%= r.getDescricao() %></strong> <br><small class="text-muted">Dia <%= r.getDiaVencimento() %> • R$ <%= r.getValor() %></small></div><div>
        <button class="btn btn-sm btn-outline-primary border-0" onclick="editarRec('<%= r.getId() %>','<%= r.getDescricao() %>','<%= r.getValor() %>','<%= r.getCategoria() %>','<%= r.getDiaVencimento() %>')"><i class="bi bi-pencil"></i></button>
        <%-- ATUALIZADO: Link para ContaRecorrenteController?acao=excluir --%>
        <button class="btn btn-sm btn-outline-danger border-0" onclick="confirmarAcao('Cancelar Assinatura', 'Deseja parar esta conta?', '${pageContext.request.contextPath}/ContaRecorrenteController?acao=excluir&id=<%= r.getId() %>')"><i class="bi bi-trash"></i></button>
    </div></li><% } %></ul><% } else { %><p class="text-center p-3 text-muted">Nenhuma conta fixa.</p><% } %></div></div></div></div>

    <%-- Offcanvas de Relatórios --%>
    <div class="offcanvas offcanvas-start" tabindex="-1" id="offRelatorios">
        ...
        <div class="offcanvas-body d-flex flex-column justify-content-center">
            <div class="text-center mb-4"><i class="bi bi-file-earmark-spreadsheet display-1 text-success"></i><h5 class="mt-3">Exportar Tabela</h5><p class="text-muted small">Referência: <%= mes %>/<%= ano %></p></div>
            <div class="mb-3 text-start"><label class="form-label small fw-bold text-muted">Nome do Arquivo (Opcional):</label><div class="input-group"><input type="text" id="nomeArquivoExcel" class="form-control" placeholder="Ex: Minha Planilha"><span class="input-group-text text-muted">.xls</span></div></div>
            <button class="btn btn-success w-100 shadow-sm py-3 mb-2" onclick="exportarTabelaExcel()"><i class="bi bi-download me-2"></i> Baixar Excel</button>
        </div>
    </div>
    ...
    <script>
        ...
        function toggleGfx() { body.classList.toggle('show-gfx'); const show = body.classList.contains('show-gfx'); btnRel.style.display = show ? 'none' : 'block'; btnV.style.display = show ? 'block' : 'none'; btnX.innerHTML = show ? '<i class="bi bi-x-lg"></i>' : '<i class="bi bi-bar-chart-line-fill fs-5"></i>'; }
        btnX.onclick = toggleGfx; btnV.onclick = toggleGfx;
        ...
        function abrirModalCriar() {
            ...
            // Preenche data com dia atual ou dia 1
            let ano = <%= ano %>, mes = <%= mes %>; let hoje = new Date(); let dia = "01";
            if (hoje.getFullYear() === ano && (hoje.getMonth() + 1) === mes) { let d = hoje.getDate(); dia = d < 10 ? "0" + d : d; }
            let mf = mes < 10 ? "0"+mes : mes;
            document.getElementById("ldata").value = ano + "-" + mf + "-" + dia;
            
            // Configura para SALVAR
            document.getElementById("ltit").innerText = "Novo Lançamento";
            document.getElementById("lacao").value = "salvar"; // Ação do Controller
            document.getElementById("formL").action = "${pageContext.request.contextPath}/TransacaoController";
        ...
        function editarT(id, desc, val, data, tipo, cat, pago) {
            ...
            document.getElementById("lacao").value = "editar"; // Configura para EDITAR

            if (parseInt(id) < 0) {
                // É recorrente: Aponta para ContaRecorrenteController
                ...
                document.getElementById("formL").action = "${pageContext.request.contextPath}/ContaRecorrenteController";
            } else {
                // É transação normal: Aponta para TransacaoController
                document.getElementById("lsplit").value = "false";
                document.getElementById("alertRec").style.display = "none";
        ...
        function editarRec(id, desc, val, cat, dia) {
            ...
            setTimeout(() => {
                ...
                document.getElementById("lacao").value = "editar";
        ...
        function confirmarAcao(titulo, texto, link) {
            ...
            document.getElementById("confMsg").innerText = texto; 
            document.getElementById("confBtn").href = link;
            new bootstrap.Modal(document.getElementById('modalConfirm')).show();
        }

        function exportarTabelaExcel() {
            // ... (Código de exportação mantido igual) ...
            var nomeInput = document.getElementById('nomeArquivoExcel').value.trim(); var nomeLimpo = nomeInput.replace(/[^a-z0-9_\-\s]/gi, '_'); var filename = nomeLimpo ? nomeLimpo + '.xls' : 'FT - Planilha Pessoal - <%= mes %>-<%= ano %>.xls'; var originalTable = document.getElementById('tabelaTransacoes'); var cloneTable = originalTable.cloneNode(true); var rows = cloneTable.rows; for (var i = 0; i < rows.length; i++) { rows[i].deleteCell(-1); if(i > 0) { var st = rows[i].cells[3]; st.innerHTML = st.getAttribute('data-status'); } } var resumo = document.createElement("table"); var r1 = resumo.insertRow(0); var c1 = r1.insertCell(0); c1.colSpan = 5; c1.innerHTML = "<b>Relatório FinanTrack - <%= mes %>/<%= ano %></b>"; c1.style.backgroundColor = "#d1e7dd"; c1.style.textAlign = "center"; c1.style.fontSize = "14px"; resumo.insertRow(1).insertCell(0).innerHTML = ""; var r2 = resumo.insertRow(2); r2.insertCell(0).innerHTML = "<b>Entradas:</b> " + document.getElementById('valEnt').innerText; r2.insertCell(1).innerHTML = "<b>Saídas:</b> " + document.getElementById('valSai').innerText; r2.insertCell(2).innerHTML = "<b>Saldo Final:</b> " + document.getElementById('valSal').innerText; resumo.insertRow(3).insertCell(0).innerHTML = ""; var html = resumo.outerHTML + cloneTable.outerHTML; html = html.replace(/ /g, '%20'); var downloadLink = document.createElement("a"); document.body.appendChild(downloadLink); if (navigator.msSaveOrOpenBlob) { var blob = new Blob(['\ufeff', html], { type: 'application/vnd.ms-excel' }); navigator.msSaveOrOpenBlob(blob, filename); } else { downloadLink.href = 'data:application/vnd.ms-excel,' + html; downloadLink.download = filename; downloadLink.click(); }
        }
    </script>
</body>
</html>